...
start = [0xFF, 0x00, 0xFF, 0xAC, 0xEB, 0x96, 0xC4, 0x2A]

def decode_bytes(rdf_data):
    current_column = 0
    pos = 0
    end_char = 0
    ...
    for char in rdf_data:
        ...
        if current_column > 7:
            current_column = 0
    return decoded_bytes[len(start):(end_char)]

def encode_bytes(xml_data):
    current_column = 0
    pos = 0
    ...
    for char in xml_data:
        ...
        if current_column > 7:
            current_column = 0
    start_bytes = bytearray(start)
    end_bytes = bytearray(column_endings[current_column])
    return start_bytes + encoded_bytes + end_bytes
        
def decode_file(rdf_file_path):
    with open(rdf_file_path, 'rb') as rdf_file:
        rdf_data = rdf_file.read()
    decoded_string = decode_bytes(rdf_data).decode('latin-1')
    output_path = rdf_file_path[:-4] + ".xml"
    with open(output_path, 'w+', encoding='latin-1') as output_file:
        output_file.write(decoded_string)

def encode_file(xml_file_path):
    with open(xml_file_path, encoding='latin-1') as xml_file:
        xml_string = xml_file.read()
    xml_data = bytearray(xml_string, encoding='latin-1')
    decoded_string = encode_bytes(xml_data)
    output_path = xml_file_path[:-4] + ".rdf"
...
def convert_file(path):
    if path[-3:] == 'rdf':
        ...
        print("Error: File extension must be .rdf or .xml")
...
if __name__ == '__main__':
    if len(sys.argv) >= 2:
        ...
        print("Error: RDF file path not specified")